...
export default function Navbar() {
  const username = localStorage.getItem("username") || "Sub Warden";
  const fullName = localStorage.getItem("fullName") || "Sub Warden";
    ...
    <header className="h-[68px] bg-white border-b border-gray-200/80 flex items-center justify-between px-6 lg:px-8">
      ...
      <div className="min-w-0">
        <h2 className="text-lg font-semibold text-gray-800">
          Sub Warden <span className="text-indigo-600">Dashboard</span>
        </h2>
        <p className="text-xs text-gray-400 -mt-0.5">
          Manage allocations, inventory checklist, and maintenance complaints
        </p>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        >
          <Bell size={18} />
          <span className="absolute top-1.5 right-1.5 w-[18px] h-[18px] bg-indigo-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center shadow-sm shadow-indigo-500/30">
            2
          </span>
        </button>
        ...
        <div className="flex items-center gap-2.5 rounded-xl hover:bg-gray-50 py-1.5 pl-1.5 pr-3 transition-colors cursor-pointer">
          ...
          <div className="hidden sm:block text-left">
            ...
            <p className="text-[11px] text-gray-400 leading-tight">@{username}</p>
          </div>
        </div>
      </div>
    </header>
