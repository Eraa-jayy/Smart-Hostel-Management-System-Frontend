...
export default function Sidebar() {
  ...
  return (
    <aside className="sticky top-0 z-30 flex h-auto w-full flex-shrink-0 self-start flex-col overflow-hidden border-b border-white/5 bg-[#0a0f1e] shadow-2xl shadow-slate-950/30 lg:h-screen lg:w-[260px] lg:border-b-0 lg:border-r">
      ...
      <nav className="flex flex-1 flex-row gap-1 overflow-x-auto px-3 py-1 lg:flex-col lg:space-y-1 lg:overflow-x-hidden lg:overflow-y-auto">
        {menus.map(({ name, path, icon: Icon }) => (
          <NavLink
            key={path}
            to={path}
            className={({ isActive }) =>
              `group flex min-h-[46px] min-w-max items-center gap-3 rounded-xl px-3 py-2.5 text-[13px] font-medium transition-all duration-200 lg:w-full lg:min-w-0 ${
                isActive
      ...
      <div className="space-y-0.5 px-3 pb-3 pt-1 lg:space-y-0.5">
